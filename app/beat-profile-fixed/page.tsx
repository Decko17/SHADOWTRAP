...
export default function BeatProfileFixedPage() {
  ...
  useEffect(() => {
    async function loadProfile() {
      try {
        ...
        setData(json);
        localStorage.setItem('shadowtrap_beat_profile', JSON.stringify(json));
      } catch (err) {
        ...
        {data && (
          <>
            ...
            <section className="border border-zinc-700 p-4 rounded-2xl">
              ...
              <ul className="list-disc pl-5">
                {data.structure?.map((item: { section: string; bars: number }, i: number) => (
            ...
            <section className="border border-zinc-700 p-4 rounded-2xl">
              <h2 className="font-semibold mb-2">Mistakes</h2>
              <ul className="list-disc pl-5">
            ...
            <Link href="/flow-lab" className="block w-full rounded-xl bg-white text-black text-center p-3 font-semibold">
              Next: Flow Lab
            </Link>
          </>
        )}
